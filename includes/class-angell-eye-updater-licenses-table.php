...
class AngellEYE_Updater_Licenses_Table extends WP_List_Table {
	...
	public function column_product_status ( $item ) {
		...
		if ( 'active' == $item['product_status'] ) {
			$deactivate_url = wp_nonce_url( add_query_arg( 'action', 'deactivate-product', add_query_arg( 'filepath', $item['product_file_path'], add_query_arg( 'page', 'angelleye-helper', network_admin_url( 'index.php' ) ) ) ), 'bulk-licenses' );
